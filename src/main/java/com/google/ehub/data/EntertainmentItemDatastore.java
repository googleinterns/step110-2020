...
public final class EntertainmentItemDatastore {
   ...
   * @return uniqueId generated by datastore
   */
  public Key addItemToDatastore(EntertainmentItem item) {
    ...
    itemEntity.setProperty(NUMBER_OF_LIKES_PROPERTY_KEY, item.getNumberOfLikes());
  ...
  /**
   * Queries entertainment items based on the number of likes.
   *
   * @param fetchOptions the fetch options used by the resulting query
   * @param sortDirection the sort direction used to order the entertainment
   *     items based on the number of likes
   * @return list with the entertainment items that match the given popularity ordering, the list
   *     will be empty if no items were found
   */
  public EntertainmentItemList queryItemsByPopularity(
      FetchOptions fetchOptions, SortDirection sortDirection) {
    return createItemListFromQuery(fetchOptions,
        new Query(ENTERTAINMENT_ITEM_KIND).addSort(NUMBER_OF_LIKES_PROPERTY_KEY, sortDirection));
  }
  ...
  private static EntertainmentItem createItemFromEntity(Entity itemEntity) {
    ...
    Long numberOfLikes = (Long) itemEntity.getProperty(NUMBER_OF_LIKES_PROPERTY_KEY);
